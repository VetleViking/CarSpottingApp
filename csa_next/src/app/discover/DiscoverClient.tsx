...
export default function DiscoverClient({
    ...
    const [sort, setSort] = useState<"recent" | "hot" | "top">(
        (searchParams.get("sort") as "recent" | "hot" | "top") || "recent"
    );
        ...
        <div className="flex flex-col gap-4 items-center mt-4 font-ListComponent">
            <div className="w-full max-w-96">
                ...
                {spots.length ? (
                    <>
                        {spots.map((item, id) => (
                            <FullSpot username={username} key={id} spot={item} isAdmin={isAdmin} />
                        ))}
                        <div className="flex justify-center m-4">
                            {reachEnd ? (
                                <p className="text-white text-xl">No more spots.</p>
                            ) : (
