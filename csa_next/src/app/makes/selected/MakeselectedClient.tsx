...
export default function MakeSelectedClient({
	altUsername,
	username,
	make,
	children,
}: MakeSelectedClientProps) {
	...
	useEffect(() => {
		const fetchData = async () => {
			...
			setData(models.sort((a: Model, b: Model) => a.model.localeCompare(b.model)));
			setLoading(false);
		};
			...
			{Array.isArray(data) && data.length > 0 ? (
				data.map((item: any, id) => (
					<div
						key={id}
						onClick={() => {
							selectedModel(item.make, item.model);
						}}
					>
						<ListComponent
							title={make == "unknown" ? item.make + " " + item.model : item.model}
						/>
					</div>
				))
			) : loading ? (
