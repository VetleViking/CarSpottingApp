...
export default async function MakeSelected({ searchParams }: SearchParams) {
    ...
    const percentageData =
        username && make != "unknown"
            ? ((await get_spotted_make_percentage(make, username)) as {
                  percentage: number;
                  numSpots: number;
                  numModels: number;
              })
        ...
        <div>
            ...
            <MakeSelectedClient altUsername={altUsername as string} username={username} make={make}>
                ...
                <p className="text-white text-center text-xl mb-4">
                    {username
                        ? `${
                              username == altUsername ? "your" : username + "'s"
                          } spots of ${make}'s:`
                        : `${make}'s models:`}
                </p>
                {username && percentageData && (
                    <p className="text-white text-center mb-4 font-ListComponent">
                        {username == altUsername ? "You" : username} have spotted{" "}
                        {percentageData.numSpots} out of the {percentageData.numModels} models in
                        the database, or {percentageData.percentage}%.
                    </p>
                )}
            </MakeSelectedClient>
        </div>
