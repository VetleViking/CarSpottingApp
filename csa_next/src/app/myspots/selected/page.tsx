...
import { get_models, get_models_redis, get_spotted_models } from "@/api/api";
...
export default function MakeSelected() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            let data = [];
            if (make == "unknown") {
                data = await get_spotted_models("", search);    
            } else {
                data = await get_spotted_models(make as string, search);
            }
