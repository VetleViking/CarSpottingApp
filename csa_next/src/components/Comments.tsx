...
const Comments: React.FC<{
    ...
    spotKey: string;
    isAdmin: boolean;
}> = ({ username, spotUsername, make, model, spotKey, isAdmin }) => {
    ...
    function CommentItem({ comment, username }: { comment: NestedComment; username: string }) {
            ...
            <div key={comment.commentId} className="px-1 mt-2 border-l-2 border-black">
                ...
                {comment.deleted != "true" && (
                    <div className="flex gap-2">
                        <p className="p-1 text-xl text-nowrap">
    ...
    const onComment = (parent?: string) => {
        ...
        add_comment(spotUsername, make, model, spotKey, newComment, parent)
            .then(() => {
                getComments();
                setNewComment("");
            })
    ...
    const getComments = () => {
        get_comments(spotUsername, make, model, spotKey)
            .then((res) => {
                setComments(res);
            })
            .catch((error) => {
    ...
    return (
        <div className="w-full max-w-96 overflow-hidden">
            ...
            >
                ...
                <DownArrow width={15} height={15} className={open ? "transform rotate-180" : ""} />
            </div>
            {open && (
