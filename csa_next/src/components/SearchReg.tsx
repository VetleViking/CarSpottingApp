...
const SearchReg = () => {
    ...
    const [regResult, setRegResult] = useState<{ make: string; model: string }>({
        make: "",
        model: "",
    });
    ...
    function searchRegHandler(reg: string) {
        ...
        regnr_info(reg).then((data: { make?: string; model?: string; error?: string }) => {
            if (data.make && data.model) {
                ...
                get_models(data.make, data.model).then((results) =>
                    setCarExists(results.length > 0)
                );
            } else {
    ...
    return (
        <div ref={dropdownRef} className="w-full cursor-pointer">
            <div
            ...
            >
                ...
                <Image
                    ...
                    width={15}
                    height={15}
                    className={regOpen ? "transform rotate-180" : ""}
                />
            </div>
            {regOpen && (
                <div className="absolute bg-black border border-[#9ca3af] p-2 w-[calc(100vw-0.5rem)] md:w-auto">
                    <div className="flex items-center justify-center gap-4">
                        ...
                        />
                        <Button onClick={() => searchRegHandler(reg)} text="Search by reg" />
                    </div>
                    <div>
                        {loading && <LoadingAnimation text="Loading" />}
                        {regResult.make && regResult.model && (
                            <div>
                                <div>
                                ...
                                <div className="flex justify-between">
                                    ...
                                    {!carExists && (
                                        <Button
                                            onClick={() => {
                                                uploadMissing(regResult.make, regResult.model).then(
                                                    () =>
                                                        (window.location.href = `/makes/selected/modelselected?make=${regResult?.make}&model=${regResult?.model}`)
                                                );
                                            }}
                        ...
                        {regError && (
                            <p className="font-ListComponent text-[#9ca3af]">Error: {regError}</p>
                        )}
                    </div>
                </div>
            )}
