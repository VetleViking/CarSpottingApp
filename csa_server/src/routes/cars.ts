...
router.get("/spots/:make/percentage", async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const percentage =
            modelsArray.length > 0 ? Math.floor((uniqueModels.size / modelsArray.length) * 100) : 0;
...
router.post("/addtag", async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const alreadyExists =
            (await redisClient.hGet("tags", tag)) || (await redisClient.hGet(`tags:${user}`, tag));
...
router.get("/regnr/:regnr", async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
        ...
        if (response.status === 401) {
            ...
            dataCar.make =
                data.kjoretoydataListe[0].godkjenning.tekniskGodkjenning.tekniskeData.generelt.merke[0].merke;
            dataCar.model =
                data.kjoretoydataListe[0].godkjenning.tekniskGodkjenning.tekniskeData.generelt.handelsbetegnelse[0];
        }
    ...
    async (req: Request, res: Response, next: NextFunction) => {
        try {
            ...
            if (!make || !model || images.length === 0) {
                return res
                    .status(400)
                    .json({ message: "Make, model, and at least one image are required" });
            }
            ...
            if (process.env.PRODUCTION !== "true") {
                // if not prod, dont try to save images
...
router.post("/likecomment", async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const alreadyLiked = await redisClient.hGet(
            `likes:comments:${user}`,
            `${key}:${commentId}`
        );
    ...
    try {
        ...
        for (const commentKey of allCommentsKeys) {
            ...
            if (comment["deleted"] === "true") {
                // comment content isnt deleted if deleted, just not sent to frontend
...
router.get("/spots/makes/:query", async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const filteredMakes = Array.from(makesArray).filter((make) =>
            make.toLowerCase().includes((query as string).toLowerCase())
        );
...
router.get(
    ...
    async (req: Request, res: Response, next: NextFunction) => {
        try {
            ...
            const combinedArray = modelsArray.map((model, index) => ({
                make: makesArray[index],
                model,
            }));
...
router.get(
    "/spots/makes/unknown/models/:query",
    async (req: Request, res: Response, next: NextFunction) => {
        try {
            const { query } = req.params;
            ...
            const filteredModels = modelsArray.filter((model) =>
                model.toLowerCase().includes((query as string).toLowerCase())
            );

            const combinedArray = filteredModels.map((model, index) => ({
                make: makesArray[index],
                model,
            }));
...
router.get("/discover", async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        if (search) {
            ...
            const filteredSpotIds = allSpots
                .map((spot, i) => {
                    const searchResult = searchSegments.every((searchString) => {
                        ...
                        const key = !stringSplit[1]
                            ? null
                            : reversed
                              ? stringSplit[0].slice(1)
                        ...
                        if (key === "user") {
                            ...
                            // if no key, search in make and model
            ...
            if (sort === "recent") {
                sortedSpotIDs = await redisClient.zRange(
                    "zset:spots:recent",
                    startIndex,
                    endIndex,
                    { REV: true }
                ...
                sortedSpotIDs = await redisClient.zRange("zset:spots:likes", startIndex, endIndex, {
                    REV: true,
                });
            } else if (sort === "hot") {
                const allSpotsKeys = await redisClient.keys(`spots:*`);
...
router.get("/search_autocomplete", async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const key =
            stringSplit[1] === undefined
                ? null
                : stringSplit[0].startsWith("!")
                  ? stringSplit[0].slice(1)
                  : stringSplit[0]; // if no value, use key as value
        ...
        if (key === "user") {
            ...
            for (const make of makesArray) {
                const filteredModels = await fetchAndFilter(
                    () => getCombinedModels(user, make),
                    value
                );
            ...
            if (!matchedMake) {
                ...
                // if found make, search in make and model
                const filteredModels = await fetchAndFilter(
                    () => getCombinedModels(user, matchedMake),
                    modelSearch
                );
        ...
        const searchStrings = searchStringsEnd.map(
            (searchString) =>
